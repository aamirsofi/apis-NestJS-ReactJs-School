...
import { FiPlus, FiEdit2, FiTrash2, FiCalendar, FiLoader, FiCheck } from 'react-icons/fi';
...
export default function AcademicYears() {
  ...
  useEffect(() => {
    if (user?.role === 'super_admin') {
      ...
      if (selectedSchoolId && selectedSchoolId !== '' && selectedSchoolId !== 'all') {
        loadAcademicYears();
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Failed to save academic year';
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-4">
                ...
                <Button
                  ...
                  disabled={user?.role === 'super_admin' && (!selectedSchoolId || selectedSchoolId === 'all')}
        ...
        {loading ? (
          <Card className="p-12">
            <CardContent className="flex items-center justify-center">
              ...
              <span className="ml-3 text-muted-foreground">Loading academic years...</span>
            </CardContent>
          </Card>
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <CardDescription className="mb-4">
                Please select a school from the dropdown above to view or manage academic years.
              </CardDescription>
            </CardContent>
          </Card>
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <CardDescription className="mb-4">Get started by creating a new academic year.</CardDescription>
              <Button
                onClick={() => {
                  resetForm();
                  setError('');
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white/50 divide-y divide-gray-200">
                  {academicYears.map((year) => (
                    <tr key={year.id} className="hover:bg-white/80 transition-smooth">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <FiCalendar className="w-5 h-5 mr-3 text-indigo-500" />
                          <div>
                            <div className="text-sm font-semibold text-gray-900">{year.name}</div>
                            {year.description && (
